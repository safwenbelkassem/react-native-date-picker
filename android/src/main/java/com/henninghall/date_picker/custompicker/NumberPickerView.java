package com.henninghall.date_picker.custompicker;
...
public class NumberPickerView extends View  {
    ...
    private NumberPickerView.OnValueChangeListener mOnValueChangeListener; //compatible for NumberPicker
    private NumberPickerView.OnScrollListener mOnScrollListener;//compatible for NumberPicker
    private NumberPickerView.OnValueChangeListenerInScrolling mOnValueChangeListenerInScrolling;//response onValueChanged in scrolling

    // The current scroll state of the NumberPickerView.
    private int mScrollState = NumberPickerView.OnScrollListener.SCROLL_STATE_IDLE;
                ...
                switch (msg.what) {
                    case HANDLER_WHAT_REFRESH:
                        if (!mScroller.isFinished()) {
                            if (mScrollState == NumberPickerView.OnScrollListener.SCROLL_STATE_IDLE) {
                                onScrollStateChange(NumberPickerView.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
                            }
                            ...
                            if (mCurrDrawFirstItemY != 0) {//need to adjust
                                ...
                                onScrollStateChange(NumberPickerView.OnScrollListener.SCROLL_STATE_IDLE);
            ...
            case MotionEvent.ACTION_UP:
                if (mFlagMayPress) {
                    ...
                    if (Math.abs(velocityY) > mMiniVelocityFling) {
                        ...
                        invalidate();
                        onScrollStateChange(NumberPickerView.OnScrollListener.SCROLL_STATE_FLING);
                    }
                    mHandlerInNewThread.sendMessageDelayed(getMsg(HANDLER_WHAT_REFRESH), 0);
    ...
    public boolean isScrolling() {
        ...
        return mScrollState == NumberPickerView.OnScrollListener.SCROLL_STATE_FLING;
    }
